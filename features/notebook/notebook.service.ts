...
import type { Notebook, NotebookCell, NotebookCellType, NotebookDetail, RunCellResponse } from '../../components/notebook/types'
...
export async function createCell(notebookId: string, payload: { type: NotebookCellType; content?: string; position?: number }) {
...
export async function updateCell(
  notebookId: string,
  payload: { cellId: string; type?: NotebookCellType; content?: string; collapsed?: boolean; position?: number }
) {
  return fetchJson<{ item: NotebookCell; cells: NotebookCell[] }>(`/api/notebooks/${encodeURIComponent(notebookId)}/cells`, {
    method: 'PATCH',
    body: JSON.stringify(payload),
...
export async function runCell(notebookId: string, cellId: string, query: string) {
  return fetchJson<RunCellResponse>(`/api/notebooks/${encodeURIComponent(notebookId)}/run-cell`, {
    method: 'POST',
    body: JSON.stringify({ cellId, query }),
  })
}
