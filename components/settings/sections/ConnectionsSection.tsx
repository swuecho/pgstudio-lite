...
export function ConnectionsSection() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async ({ item }) => {
      ...
      if (item.isDefault || connections.length === 0) setConnectionName(item.name)
      setNewName('')
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async ({ item }) => {
      ...
      if (editingId) {
        ...
        if (current?.name === connectionName || item.isDefault) setConnectionName(item.name)
      }
      setEditingId(null)
      setEditName('')
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async (_result, id) => {
      ...
      if (deleted?.name === connectionName) {
        ...
        if (fallback) setConnectionName(fallback.name)
      }
      setErrorText('')
