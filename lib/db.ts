...
import {
  ...
  queryHistory,
  querySnippets,
  tableEditorBookmarks,
  tableEditorRecentViews,
} from '../drizzle/schema'
...
export function saveTableEditorBookmark(
  ...
  if (existing) {
    metaDb
      ...
      .run()
    const row = metaDb.select().from(tableEditorBookmarks).where(eq(tableEditorBookmarks.id, existing.id)).get()
...
export function updateTableEditorBookmark({
  ...
  const result = metaDb
    .update(tableEditorBookmarks)
    .set(values)
    .where(and(eq(tableEditorBookmarks.id, id), eq(tableEditorBookmarks.connectionName, resolvedConnectionName)))
    .run()
  if (!result.changes) return null
